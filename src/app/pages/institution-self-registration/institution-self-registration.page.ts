...
import { searchFilter } from 'src/app/customPipes/ng-search-pipe';
import * as $ from 'jquery';


@Component({
  selector: 'app-institution-self-registration',
  templateUrl: './institution-self-registration.page.html',
  styleUrls: ['./institution-self-registration.page.scss'],
})
export class InstitutionSelfRegistrationPage extends LoginService {
...
validationMessage = {
  ...
  submit() {
    if (this.institutionForm.invalid) {
      this.loader.presentToast('warning', 'Form Incomplete');
      return;
    } else {
      this.success = true;
    ...
    this.sendRequestToServer(
      ...
      function(response) {
        ...
        if (response.returnCode === 0) {
          ...
          // let hfb = controller.bankList.find(bank => bank.name === 'HOUSING FINANCE BANK');
          // controller.accountArrayControl.controls.banksId.setValue(hfb);
